...
the mutt convention — unless the body already contains a separator line
(legacy drafts composed with embedded footers are sent as-authored, never
double-footered). Sent mail is never copied into cards: notmuch holds
...
def flush(
    ...
    *,
    ...
    if footer_path.is_file() and not re.search(r"^-- $", card.body, re.M):
        card.body = (
